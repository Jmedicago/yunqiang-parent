...
``highlight(text, value, ignore_illegals, continuation)``
---------------------------------------------------------

Core highlighting function.
Accepts a language text, or an alias, and a string with the code to highlight.
...
* ``language``: language text, same as the one passed into a function, returned for consistency with ``highlightAuto``
...
``registerLanguage(text, language)``
------------------------------------

Adds new language to the library under the specified text. Used mostly internally.

* ``text``: a string with the text of the language being registered
...
Looks up a language by text or alias.
